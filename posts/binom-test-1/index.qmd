...
# calculate p2 probabilities
dt[p <= 0.5, binom.p2 := pbinom(Sucesses, size=N, prob=p) * 100]
dt[p <= 0.5, norm.p2 := (1-dnorm(Sucesses, mean=N * p, sd = sqrt(p * (1 - p) * N)))/2 * 100]
dt[p > 0.5, binom.p2 := pbinom(Failures, size=N, prob=p) * 100]
dt[p > 0.5, norm.p2 := (1-dnorm(Failures, mean=N * p, sd = sqrt(p * (1 - p) * N)))/2 * 100]

# calculate p3 probabilities
dt[p <= 0.5, binom.p3 := (1 - pbinom(Sucesses, size=N, prob=p)) * 100]

# get ratios between estimates
