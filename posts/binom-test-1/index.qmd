...
Another way to measure approximation error is to calculate total absolute approximation error (binomial minus normal). The reason why one needs to use total is that mean absolute error would be effected by very small errors at the edge of distributions. The main idea is that by increasing number of observations (expected number of success/failures) total error between point estimates will get reduced. As with previous example, total approximation error is calculated for different PDs with 5 expected successes/failures.
...
From table above one can observe that total point approximation error is the smallest for the largest probability (50%). For very likely, e.g. 99.9%, or unlikely (0.1%) situations approximating binomial distribution with normal leads to >10% point estimate errors.
...
Now let's investigate how these probabilities and errors change as sample size is increased and "success-failure" condition is met. For some probabilities these curves are not monotonous, because, you can get exactly 5 expected values with p = 35%. For such cases sample size was floored to nearest integer number, e.g. for 35% probability to get 5 EV one needs X tries. In addition aproximation error was calculated for all cases.
