...
The proportion of imports in the total volume of the European Union has increased by 50%, rising from around 50% to approximately 75%. Similarly, there have been noticeable changes in exports to EU countries. Meanwhile, exports and imports to Russia had been steadily declining even before the start of the 2022 war. Before the start of the war, for few years Lithuania was net importer with Russian trade. Even though there were tensions between China and Lithuania no notable changes could be observed to exports to China. In fact, the share to total imports from China increased almost 5 times

Finally let's analyze how import and exports were affected with Countries close to Russia.
...
# color dictionary
color.dict <- c("Kazakhstan"= "#C5AFA4", "Belarus" = "#CC7E85",
                "Russia" = "#CF4D6F", "Turkey" = "#A36D90",
                "Offshore financial centers" = "#76818E")

# left figure
fig1 <- plot_ly(data = dt.plot.3[Type == "Imports"],
                x = ~Quarter, y = ~value, color=~Country, colors = color.dict,
                type = 'scatter', mode = 'lines+markers',
...
fig2 <- plot_ly(data = dt.plot.3[Type == "Exports"],
                x = ~Quarter, y = ~value, color=~Country,
                colors = color.dict,
                type = 'scatter', mode = 'lines+markers',
                line = list(dash = 'dot'),
                marker = list(line = list(width = 3))) %>%
...
fig <- subplot(fig1, fig2, nrows = 2) %>% 
  layout(title = 'Countries friendly to Russia')
fig
```
...
This analysis could be used to evaluate the impact of various historical events to Lithuania's exports and imports. Following conclusions can be drawn:

* Declines of imports and exports to Russia started way before of 2022 invasion.
*
*
